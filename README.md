...
4. Python webserver (endpoints & webhook)
...
```
Browser ──HTTP──> Python Server ──Serial──> ESP32 ──> LED Strip
GitHub  ──Webhook──> Python Server (signature + commit checks)
```
...
autossh -M 0 -N -o "ServerAliveInterval 30" -o "ServerAliveCountMax 3" \
	-R 0.0.0.0:55155:localhost:55155 user@remote.example.org
```

Security: Only `/webhook` enforces signatures. Consider protecting other endpoints if exposed publicly.
